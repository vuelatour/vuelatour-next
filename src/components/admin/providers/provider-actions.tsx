...
import { deleteProviderAction } from "@/app/admin/providers/actions";
...
export function ProviderActions({ provider }: { provider: Provider }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      ...
      if (result.ok) {
        toast.success("Proveedor marcado como inactivo");
        setOpenDelete(false);
      } else {
  ...
  return (
    <>
      ...
      <ProviderFormDialog open={openEdit} onOpenChange={setOpenEdit} initialProvider={provider} />

      <AlertDialog open={openDelete} onOpenChange={setOpenDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar proveedor {provider.nombre}?</AlertDialogTitle>
            <AlertDialogDescription>
              Se marcará como inactivo. Los gastos y facturas históricas quedan intactos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
