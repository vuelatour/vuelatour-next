...
import { deleteBankAccountAction } from "@/app/admin/bank-accounts/actions";
import { BankAccountFormDialog } from "./bank-account-form-dialog";
import type { BankAccount } from "@/types/bank-accounts";

export function BankAccountActions({ account }: { account: BankAccount }) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      ...
      if (result.ok) {
        toast.success("Cuenta marcada como inactiva");
        setOpenDelete(false);
      } else {
  ...
  return (
    <>
      ...
      <AlertDialog open={openDelete} onOpenChange={setOpenDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar cuenta {account.alias}?</AlertDialogTitle>
            <AlertDialogDescription>
              Se marcará como inactiva. Los movimientos históricos quedan intactos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
