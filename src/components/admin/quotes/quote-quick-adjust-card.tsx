...
export function QuoteQuickAdjustCard({ quote }: { quote: PersistedQuote }) {
  ...
  const [extras, setExtras] = useState<ExtraConcepto[]>(quote.extras ?? []);
  ...
  const add = (concepto = "") =>
    setExtras((prev) => [...prev, { concepto, monto_usd: 0, aplica_iva: true }]);
  ...
  const cambioExtras =
    JSON.stringify(extrasValidos) !== JSON.stringify(quote.extras ?? []);
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex justify-end pt-1">
          <Button onClick={handleSave} disabled={pending || !hayCambios}>
